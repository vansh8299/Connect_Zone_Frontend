...
import { SEND_OTP_MUTATION, VERIFY_OTP_MUTATION, SIGNUP_MUTATION } from '@/graphql/query/query';
    ...
    onSubmit: async (values) => {
      ...
      try {
        ...
        if (data?.sendOtp.success) {
          ...
          setErrorMessage(data?.sendOtp.message || 'Failed to send OTP. Please try again.');
        }
      } catch (error: any) {
    ...
    onSubmit: async (values) => {
      ...
      try {
        ...
        if (data?.verifyOtp.success) {
          ...
          setErrorMessage(data?.verifyOtp.message || 'OTP verification failed. Please try again.');
        }
      } catch (error: any) {
  ...
  const handleResendOtp = async () => {
    ...
    try {
      ...
      if (data?.sendOtp.success) {
        ...
        setErrorMessage(data?.sendOtp.message || 'Failed to resend OTP. Please try again.');
      }
    } catch (error: any) {
  ...
  const completeSignup = async () => {
    ...
    try {
      ...
      if (data?.signup) {
        ...
        setCookie('user', JSON.stringify(data.signup.user), {
          maxAge: 60 * 60 * 24 * 7, // 1 week
          secure: process.env.NODE_ENV === 'production',
          sameSite: 'strict',
          path: '/',
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-indigo-50 to-blue-100 flex items-start justify-center p-4">
      ...
      >
        <div className="bg-white rounded-2xl shadow-xl overflow-hidden">
            ...
            <div className="flex items-center justify-between mb-4">
              <div className="flex flex-col items-center">
                <div className={`w-8 h-8 rounded-full flex items-center justify-center ${
                  currentStep === SignupStep.INITIAL_INFO ? 'bg-indigo-600 text-white' : 'bg-indigo-100 text-indigo-600'
                }`}>
                  1
                </div>
              ...
              <div className={`h-1 flex-grow mx-2 ${
                currentStep === SignupStep.INITIAL_INFO ? 'bg-gray-200' : 'bg-indigo-400'
              }`} />
              <div className="flex flex-col items-center">
                <div className={`w-8 h-8 rounded-full flex items-center justify-center ${
                  currentStep === SignupStep.OTP_VERIFICATION ? 'bg-indigo-600 text-white' : 
                  currentStep === SignupStep.COMPLETE_SIGNUP ? 'bg-indigo-100 text-indigo-600' : 'bg-gray-200 text-gray-500'
                }`}>
                  2
                </div>
              ...
              <div className={`h-1 flex-grow mx-2 ${
                currentStep === SignupStep.COMPLETE_SIGNUP ? 'bg-indigo-400' : 'bg-gray-200'
              }`} />
              <div className="flex flex-col items-center">
                <div className={`w-8 h-8 rounded-full flex items-center justify-center ${
                  currentStep === SignupStep.COMPLETE_SIGNUP ? 'bg-indigo-600 text-white' : 'bg-gray-200 text-gray-500'
                }`}>
                  3
                </div>
          ...
          {currentStep === SignupStep.INITIAL_INFO && (
            <form onSubmit={initialFormik.handleSubmit} className="p-8 pb-4 space-y-2">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  {initialFormik.touched.firstName && initialFormik.errors.firstName && (
                    <p className="mt-1 text-sm text-red-600">{initialFormik.errors.firstName}</p>
                  )}
                </div>
                ...
                <div>
                  ...
                  {initialFormik.touched.lastName && initialFormik.errors.lastName && (
                    <p className="mt-1 text-sm text-red-600">{initialFormik.errors.lastName}</p>
                  )}
                </div>
              </div>
              ...
              <div>
                ...
                {initialFormik.touched.email && initialFormik.errors.email && (
                  <p className="mt-1 text-sm text-red-600">{initialFormik.errors.email}</p>
                )}
              </div>
              ...
              <div>
                ...
                {initialFormik.touched.password && initialFormik.errors.password && (
                  <p className="mt-1 text-sm text-red-600">{initialFormik.errors.password}</p>
                )}
              </div>
              ...
              <div>
                ...
                {initialFormik.touched.confirmPassword && initialFormik.errors.confirmPassword && (
                  <p className="mt-1 text-sm text-red-600">{initialFormik.errors.confirmPassword}</p>
                )}
              </div>
            ...
            <form onSubmit={otpFormik.handleSubmit} className="p-8 space-y-4">
              <div className="text-center mb-4">
                <p className="text-gray-600">
                  ...
                  <span className="font-medium text-gray-900">{initialFormik.values.email}</span>
                </p>
              </div>
              ...
              <div>
                ...
                {otpFormik.touched.otp && otpFormik.errors.otp && (
                  <p className="mt-1 text-sm text-red-600">{otpFormik.errors.otp}</p>
                )}
              </div>
              ...
              <div className="flex items-center justify-between mt-4">
                ...
                <button
                  ...
                  className={`text-sm ${resendDisabled ? 'text-gray-400 cursor-not-allowed' : 'text-indigo-600 hover:text-indigo-500'}`}
                >
                  {resendDisabled 
     ...
     <div className="mt-3">
              ...
              <div className="mt-2">
                <GoogleAuthButton text="Sign up with Google" onError={handleGoogleError} />
              </div>
            </div>
